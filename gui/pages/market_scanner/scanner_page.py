...
# ─────────────────────────────────────────────────────────────
# Agent Assessment Helper
# ─────────────────────────────────────────────────────────────

# Per-agent interpretation of what positive vs negative signal means.
# Each entry: (positive_meaning, negative_meaning, neutral_meaning)
...
# Tells the user what the agent measures so they understand its purpose.
...
def _make_assessment(agent_name: str, signal: float, conf: float,
    ...
    # Visible cell text: conviction level + direction + what it means
    visible_text = f"{conf_label} · {direction}\n{meaning}"

    # Hover tooltip: full numeric breakdown
    full_tooltip = (
...
# ─────────────────────────────────────────────────────────────
# Intelligence Agents Dashboard Tab
# ─────────────────────────────────────────────────────────────
class _AgentRow(QFrame):
    ...
    def __init__(self, agent_name: str, parent=None):
        ...
        # Agent name
        self._name_lbl = QLabel(agent_name.replace("_", " ").title())
        self._name_lbl.setFixedWidth(160)
        self._name_lbl.setStyleSheet("font-weight:600; color:#C8D0E0;")
        ...
        self._assess_lbl.setStyleSheet(
            "color:#6B7FA3; font-size:13px;"
        )
        self._assess_lbl.setWordWrap(True)
        self._assess_lbl.setAlignment(Qt.AlignLeft | Qt.AlignVCenter)
        h.addWidget(self._assess_lbl, 1)   # takes all remaining stretch

        # Last updated — fixed width now that assessment owns the stretch
    ...
    def refresh(self, status: dict) -> None:
        ...
        # Colour: match signal direction; white when no data (awaiting)
        if conf == 0.0 and signal == 0.0:
            assess_color = "#E8EBF0"
        elif signal > 0.05:
            assess_color = "#48BB78"
        ...
        self._assess_lbl.setStyleSheet(
            f"color:{assess_color}; font-size:13px;"
        )
        self._assess_lbl.setToolTip(tooltip)

        # Timestamp: show only time part
        if "T" in updated:
...
class AgentsDashboardTab(QWidget):
    """
    ...
    def __init__(self, parent=None):
        ...
        # Poll coordinator for status every 5 seconds via QTimer
        from PySide6.QtCore import QTimer
        ...
        # Subscribe to OrchestratorEngine signal
        from core.event_bus import bus, Topics
...
class MarketScannerPage(QWidget):
    ...
    def _build(self):
        ...
        # Tab 3: Intelligence Agents dashboard
        self._agents_tab = AgentsDashboardTab()
        self._tabs.addTab(self._agents_tab, "  ◉  Intelligence Agents  ")
