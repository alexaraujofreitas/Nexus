...
class BacktestingPage(QWidget):
    ...
    def _build(self):
        ...
        layout.addWidget(PageHeader(
            "IDSS Backtesting",
            "Evaluate IDSS strategies using real historical exchange data"
        ))
    ...
    def _build_toolbar(self) -> QFrame:
        ...
        # Hidden stubs for backward compat with methods that reference these
        self._strat_combo = QComboBox(); self._strat_combo.setVisible(False)
        self._strat_info_lbl = QLabel(""); self._strat_info_lbl.setVisible(False)
        self._run_btn = QPushButton(); self._run_btn.setVisible(False)
        self._promote_btn = QPushButton(); self._promote_btn.setVisible(False)
        self._optimize_btn = QPushButton(); self._optimize_btn.setVisible(False)

        # Capital — uses _FormattedSpinBox for comma-formatted display (100,000)
        ...
        self._capital_spin.setRange(100, 10_000_000); self._capital_spin.setValue(100_000)
        self._capital_spin.setDecimals(0); self._capital_spin.setSuffix(" USDT")
        self._capital_spin.setFixedWidth(160)
        row1.addWidget(self._capital_spin)

        row1.addSpacing(8)

        # Fee spinbox — pre-filled with Bybit standard 0.10% maker & taker
        ...
        self._fee_spin.setValue(0.10)          # Bybit standard spot 0.10%
        ...
        self._fee_spin.setToolTip(
            ...
            "Bybit standard: 0.10% maker & 0.10% taker"
        )
        ...
        # Hidden stub for backward compat
        self._fetch_btn = QPushButton(); self._fetch_btn.setVisible(False)
        ...
        self._save_btn.setToolTip("Save the current backtest result for later comparison and analysis.")
        self._save_btn.setStyleSheet(
            "QPushButton { background:#2A3040; color:#AABBCC; border:1px solid #3A4050; "
            "border-radius:5px; font-size:12px; font-weight:700; padding:0 16px; }"
            "QPushButton:hover { background:#3A4050; color:#DDEEEE; border-color:#5A6070; }"
            "QPushButton:disabled { background:#1A2030; color:#555; border-color:#2A3040; }"
        )
        self._save_btn.clicked.connect(self._save_backtest_result)
    ...
    def _load_idss_symbols(self):
        """Populate the IDSS symbol combo from the active scanner watchlist."""
        symbols = []
        # Primary: use scanner watchlist (same symbols the live IDSS scans)
        try:
            from core.scanning.watchlist import WatchlistManager
            wm = WatchlistManager()
            symbols = sorted(wm.get_active_symbols())
        except Exception:
            pass
    ...
    def _run_idss_backtest(self):
        ...
        # Load OHLCV from exchange (Bybit primary, Binance fallback)
        try:
            from core.backtesting.data_loader import HistoricalDataLoader

            self._status_lbl.setText(f"Fetching {symbol} {timeframe} data from exchange...")
            ...
            if df is None or df.empty:
                self._status_lbl.setText(f"⚠ No data available for {symbol} {timeframe}")
                return

            # Get data source metadata
            data_info = loader.get_last_fetch_info()
            earliest = df.index[0].strftime("%Y-%m-%d")
            latest = df.index[-1].strftime("%Y-%m-%d")
            days = (df.index[-1] - df.index[0]).days
            source = data_info.fallback_source if data_info.fallback_used else data_info.primary_source
            self._data_range_lbl.setText(
                f"Data: {earliest} → {latest}  |  {len(df):,} candles  |  ~{days} days  |  Source: {source}"
            )

        except Exception as exc:
            self._status_lbl.setText(f"⚠ Data fetch error: {exc}")
            return
    ...
    def _on_idss_finished(self, result: dict):
        ...
        try:
            ...
            kpis = compute_kpis(
                ...
                total_bars=result.get("candle_count", 0),
                timeframe=self._idss_tf_combo.currentData() or "1h",
            )
            self._current_kpis = kpis
            self._save_btn.setEnabled(True)
        except Exception as exc:
            logger.warning("KPI computation failed: %s", exc)
            self._current_kpis = None
    ...
    def _save_backtest_result(self):
        """Save the current backtest result for later comparison and analysis."""
        if not self._current_result:
            return

        try:
            from core.backtesting.result_store import BacktestResultStore, BacktestResult
            from core.backtesting.kpi_engine import kpis_to_dict
            ...
            symbol = self._idss_symbol_combo.currentText()
            timeframe = self._idss_tf_combo.currentData() or "1h"
            start_date_str = self._start_date.date().toString("yyyy-MM-dd")
            end_date_str = self._end_date.date().toString("yyyy-MM-dd")
            ...
            result = BacktestResult(
                ...
                name=f"{symbol}_{timeframe}_{start_date_str}",
                strategy_name="IDSS",
                symbol=symbol,
                timeframe=timeframe,
                ...
                regime_affinity={},
                global_params={},
                kpis=kpis_to_dict(self._current_kpis) if self._current_kpis else {},
                trades=self._current_result.get("trades", []),
                equity_curve=self._current_result.get("equity_curve", []),
                equity_timestamps=self._current_result.get("equity_timestamps", []),
                total_bars=self._current_result.get("candle_count", 0),
                warmup_bars=100,
            )

            result_id = store.save(result)
            self._status_lbl.setText(f"✓ Result saved (ID: {result_id[:8]}...)")
            QMessageBox.information(self, "Saved", f"Backtest result saved.\nID: {result_id}")

        except Exception as exc:
            logger.error("Failed to save backtest result: %s", exc)
            QMessageBox.warning(self, "Save Error", f"Could not save result:\n{exc}")
