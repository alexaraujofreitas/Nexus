...
async function fetch(url, opts = {}) {
  for (let i = 0; i < 3; i++) {
    try {
      ...
      const retryable = msg.includes('ECONNRESET') || msg.includes('timeout') || msg.includes('socket hang up');
      if (i < 2 && retryable) {
        await sleep(1000 * (i + 1));
        continue;
      }
      // Return a synthetic error response instead of crashing
      return { status: 503, headers: {}, body: `fetch error: ${msg}`, ok: false };
    }
  }
}
...
async function test01_LoginFlow() {
  ...
  if (login.status === 200 && login.json?.access_token) {
    ...
    // Try setup (first-run user creation)
    const reg = await fetchJSON(`${API}/api/v1/auth/setup`, {
      method: 'POST',
      body: JSON.stringify({ email: 'test@nexustrader.dev', password: 'TestPass123!' }),
    });
  ...
  if (accessToken) {
    const authed = await authedFetch(`${API}/health`);
...
async function test07_Analytics() {
  ...
  // Verify analytics tab query param renders (Phase 5)
  for (const tab of ['equity', 'trades', 'models', 'regime']) {
    const tabRes = await fetch(`${BASE}/analytics?tab=${tab}`);
    if (tabRes.ok) pass(`Analytics tab=${tab} route returns 200`);
    else fail(`Analytics tab=${tab} route returns 200`, `Got ${tabRes.status}`);
  }

  // Verify all analytics engine endpoints (Phase 4 + Phase 5)
  const endpoints = [
    ...
    { name: 'Drawdown curve', action: 'get_drawdown_curve' },
    { name: 'Rolling metrics', action: 'get_rolling_metrics' },
    { name: 'R-distribution', action: 'get_r_distribution' },
    { name: 'Duration analysis', action: 'get_duration_analysis' },
    { name: 'By regime', action: 'get_performance_by_regime' },
    { name: 'Regime transitions', action: 'get_regime_transitions' },
  ];
...
async function test08_Backtest() {
  ...
  // Phase 5: Verify cancel endpoint
  const cancel = await authedFetch(`${API}/api/v1/backtest/cancel/test-job-1`, {
    method: 'POST',
  });
  if (cancel.status !== 401) pass('Backtest cancel API accessible');
  else fail('Backtest cancel API accessible', `Got ${cancel.status}`);
}
...
async function test10_MobileAndRoutes() {
  ...
  // Verify CSS delivery mechanism (Vite dev uses HMR via /@vite/client, prod uses <link>)
  const links = doc.querySelectorAll('link[rel="stylesheet"]');
  const viteClient = doc.querySelector('script[src*="@vite/client"]');
  const hasCSS = links.length > 0 || doc.querySelectorAll('style').length > 0 || viteClient;
  if (hasCSS) pass('CSS delivery mechanism present (Vite HMR or <link>)');
  else fail('CSS delivery mechanism present');
