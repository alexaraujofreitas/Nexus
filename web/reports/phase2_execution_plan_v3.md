# Phase 2 Execution Plan v3 — Market Data Service + Snapshot Pipeline

**Status:** PENDING APPROVAL — do NOT implement until explicitly authorized.
**Date:** 2026-04-03
**Revision:** v3 — fixes source_candle_ts derivation (hour-boundary deterministic), snapshot write atomicity (HSET field-scoped), tier race conditions (single-writer assembler), snapshot TTL (600s), explicit write ordering, and updated tests. No other changes from v2.
...
├── _snapshot_lock: asyncio.Lock               (serializes snapshot assembly)
│
├── start() → spawns tier tasks
├── stop() → cancels all tasks, closes CCXT
│
├── _tier0_loop()           # price-only: fetch_tickers → enqueue update → assembler flushes
├── _tier1_loop()           # 1H metrics: fetch 1h OHLCV → Redis OHLCV cache → enqueue update
├── _tier2_loop()           # deep metrics: fetch 1d OHLCV → Redis OHLCV cache → enqueue update → DB persist
│
├── _assemble_and_flush(asset_id, tier, fields: dict) → None
│   # THE SINGLE WRITER: acquires _snapshot_lock, merges fields into _snapshots[asset_id],
│   # writes Redis HASH, publishes WS, optionally persists to DB
│
...
**Problem:** In v2, Tier 0, Tier 1, and Tier 2 each independently performed GET → merge → SET on the snapshot key. Because these are separate `asyncio.Task`s sharing one event loop, interleaving is possible: Tier 0 GETs, Tier 1 GETs (same stale copy), Tier 0 SETs (overwrites with price fields), Tier 1 SETs (overwrites Tier 0's price update with its own merge that used the pre-Tier-0 price). Result: Tier 0's price update is lost.

**Solution: In-memory canonical snapshot with single-writer flush (Option C from review).**

Every tier computes its field values, then calls `_assemble_and_flush(asset_id, tier, fields)`. This method:

1. Acquires `_snapshot_lock` (asyncio.Lock — all tiers run on the same event loop, so this serializes access without blocking the loop).
2. Merges `fields` into `_snapshots[asset_id]` (the in-memory canonical dict). Only the keys present in `fields` are overwritten — other keys are preserved.
3. Sets `computed_at` to `utcnow()`.
4. Writes the full snapshot to Redis HASH (`HSET` with all 16 fields).
5. Publishes the snapshot to `nexus:events:ticker`.
6. If `tier == 2`: also persists to PostgreSQL.
7. Releases lock.

**Why this prevents all race conditions:**
- Only `_assemble_and_flush` writes to `_snapshots[asset_id]` and to Redis. No other code path writes snapshots.
- The lock serializes all writes. If Tier 0 and Tier 1 both call `_assemble_and_flush` near-simultaneously, one completes fully before the other starts.
- No GET-before-SET pattern exists — the canonical state is in memory, not read from Redis.
- Stale data regression is impossible because each tier only overwrites its own fields (price fields for Tier 0, 1h-metrics for Tier 1, deep metrics for Tier 2).
...
2. `start()` queries DB for active exchange, initializes CCXT instance (demo mode, `enableRateLimit: True`, 15s timeout, `rateLimit: 350`), calls `load_markets()`, initializes `_snapshots` dict for all tradable assets (empty dicts), then spawns three `asyncio.Task`s: `_tier0_loop`, `_tier1_loop`, `_tier2_loop`.
...
| Redis write failure | Log error, skip WS delivery. DB persist still attempted. `_snapshots` in-memory dict remains valid. |
...
All values are scalars (float, string, or null). No nested objects.
...
_tier0_loop:
    while running:
        t0 = time.monotonic()
        tickers = await _fetch_tickers(tradable_symbols)
        for asset_id, ticker in tickers:
            ...
            await _assemble_and_flush(asset_id, tier=0, fields=fields)
...
_tier1_loop:
    while running:
        t0 = time.monotonic()
        for asset in tradable_assets (semaphore-bounded):
...
```
async def _assemble_and_flush(asset_id: int, tier: int, fields: dict) -> None:
    async with self._snapshot_lock:
        # 1. COMPUTE: merge fields into in-memory canonical snapshot
        snap = self._snapshots.setdefault(asset_id, {})
        snap.update(fields)
        snap["computed_at"] = datetime.utcnow().isoformat() + "Z"

        # 2. WRITE REDIS: atomic HSET of all 16 fields
        key = f"nexus:snapshot:{asset_id}"
        await self._redis.hset(key, mapping={k: json.dumps(v) for k, v in snap.items()})
        await self._redis.expire(key, SNAPSHOT_TTL_S)

        # 3. PUBLISH: push full snapshot to WS via Redis pub/sub
        payload = json.dumps({
            "type": "snapshot_update",
            "tier": tier,
            "exchange_id": self._exchange_db_id,
            "assets": [{"asset_id": asset_id, "symbol": self._symbols[asset_id], "snapshot": snap}],
            "timestamp": snap["computed_at"],
        })
        await self._redis.publish("nexus:events:ticker", payload)

        # 4. PERSIST DB (Tier 2 only): flush to PostgreSQL
        #    (actual DB write happens in _tier3_persist after all assets are assembled)
```

**Write order is always: compute → Redis HSET → Redis PUBLISH → DB (Tier 2 only).** This is explicit and invariant across all tiers.
...
- **Cross-tier isolation:** Tier 0 never waits for Tier 1/2. Tier 1 never waits for Tier 2. Each task is independent. The only serialization point is the `_snapshot_lock`, which is held for microseconds (in-memory merge + two Redis commands).
...
CCXT (exchange) → MDS (compute) → _snapshots (in-memory canonical)
                                       ↓
                                  Redis HASH (hot authority, HSET projection)
                                       ↓
...
|------------|------|---------|-----|
| `nexus:snapshot:{asset_id}` | HASH | 16 fields, each a JSON-encoded value | **600s** (renewed on every write) |
| `nexus:snapshot:all:{exchange_id}` | STRING | JSON dict `{asset_id: snapshot}` for bulk reads | **600s** (renewed on every Tier 1/2 write) |

**Why TTL = 600s (not 300s):** The longest write interval is Tier 2 at 300s. A 300s TTL risks expiry during a slow Tier 2 cycle (e.g., 290s cycle + 15s network hiccup = 305s). A 600s TTL provides 2× headroom. If MDS is truly down for >10 minutes, the TTL expiry correctly signals "no fresh data" to REST consumers, who then fall back to DB.
...
Note: `HSET` writes only the fields provided in the current tier's update. Fields from other tiers are already present and untouched (HASH semantics: HSET only overwrites the specified fields). However, because we use the single-writer assembler, the full 16-field set is always written via `hset(key, mapping=...)` to keep the HASH and in-memory dict identical.
...
```
1. COMPUTE  — merge tier-specific fields into _snapshots[asset_id] in memory
2. REDIS    — HSET nexus:snapshot:{asset_id} (all 16 fields) + EXPIRE 600
3. PUBLISH  — PUBLISH nexus:events:ticker (full snapshot payload)
4. DB       — (Tier 2 only) UPDATE assets SET market_snapshot = ..., snapshot_updated_at = now()
              + INSERT INTO ohlcv ... ON CONFLICT DO UPDATE
```

This order ensures:
- Redis is always updated before pub/sub (WS subscribers never see a notification without the Redis key being current).
...
| `GET /market-data/snapshots` | Redis `HGETALL` for each tradable asset (or `GET nexus:snapshot:all:{eid}` bulk key) | PostgreSQL `SELECT market_snapshot FROM assets WHERE is_tradable` | Redis first; if empty/expired, fall back to DB |
...
2. **REST response:** On initial page load or reconnection, frontend calls `GET /market-data/snapshots`. Handler reads Redis first; if Redis is empty or expired, reads PostgreSQL. Response includes `computed_at` — frontend displays "stale" indicator if `computed_at` is older than 5 minutes.
...
Returns latest snapshots for all tradable assets. Reads Redis HASH first, falls back to PostgreSQL.
...
`"data_source": "redis"` or `"data_source": "db"` indicates which layer served the snapshot. Observability only — frontend ignores it.
...
`"tier"` indicates which tier produced this update (0, 1, or 2). The `snapshot` object always contains all 16 fields (some may be `null` during staged warm-up). This lets the frontend always overwrite its local state with the latest server-side snapshot — no field-level merge logic on the client.
...
Compares against `_known_asset_ids`. Any new `asset_id` triggers staged warm-up.

### Staged Warm-Up Timeline

| Stage | Trigger | Latency from toggle | What happens | Metrics available after |
|-------|---------|--------------------|--------------|-----------------------|
| **Stage 0** (detection + price) | Next Tier 0 cycle | 0–15s | New asset_id detected. Added to `_known_asset_ids` and `_snapshots`. `fetch_tickers` already includes it. `_assemble_and_flush(asset_id, tier=0, price_fields)`. | `price`, `bid`, `ask`, `spread_pct`, `last_trade_ts`, `source`, `computed_at`. All change/volume/high/low fields = `null`. `source_candle_ts` = `null`. |
| **Stage 1** (1H backfill) | Next Tier 1 cycle | 15s–75s | `fetch_ohlcv(symbol, "1h", limit=168)` — full 7-day backfill. ZADD into Redis 1H sorted set. Compute Tier 1 metrics. `_assemble_and_flush(asset_id, tier=1, tier1_fields)`. | `change_1h`, `change_4h`, `volume_1h`, `volume_4h`, `high_24h`, `low_24h`, `source_candle_ts`. `change_12h`/`change_24h`/`change_7d`/`volume_24h` still `null`. |
| **Stage 2** (1D backfill + DB persist) | Next Tier 2 cycle | 75s–375s | `fetch_ohlcv(symbol, "1d", limit=30)` — full 30-day backfill. ZADD into Redis 1D sorted set. Compute Tier 2 metrics. `_assemble_and_flush(asset_id, tier=2, tier2_fields)`. Tier 3 persists to DB. | ALL 16 fields populated. Full snapshot. |

### UI-Visible Timeline

- **Within 15 seconds:** User sees the asset appear with a live price, bid/ask, and spread. Change and volume columns show "—".
- **Within ~75 seconds:** 1H metrics populate. User sees change_1h, change_4h, hourly volumes, 24h high/low.
...
1. Asset removed from `_known_asset_ids` and `_snapshots` on next Tier 0 cycle.
...
- `fakeredis.aioredis.FakeRedis` for Redis (supports sorted sets, HASH, pub/sub, TTL, EXPIRE).
...
| R3 | `test_snapshot_hset_and_hgetall` | HSET 16 fields → HGETALL returns identical dict |
...
| R7 | `test_snapshot_all_key_consistency` | `nexus:snapshot:all:{eid}` matches individual HGETALL results |
...
| A5 | `test_snapshot_integrity_under_rapid_tier0` | Fire 10 Tier 0 updates in tight loop. After all complete, HGETALL returns a consistent 16-field snapshot with monotonically increasing `computed_at`. |
...
| E3 | `test_redis_write_failure_db_still_persists` | Redis HSET raises → DB write still attempted and succeeds |
...
4. `GET /market-data/snapshots` → 3 snapshots, all 16 fields non-null, `data_source: "redis"`.
...
| F4 | All 16 snapshot fields computed correctly | Tests U1–U14 all passing |
...
| `nexus:snapshot:{asset_id}` | HASH | 16 snapshot fields, each JSON-encoded | 600s (renewed on write) | MDS assembler (all tiers) | REST /snapshots/{id} |
| `nexus:snapshot:all:{exchange_id}` | STRING | JSON dict `{asset_id: snapshot}` for bulk reads | 600s (renewed on Tier 1/2 write) | MDS assembler | REST /snapshots |
